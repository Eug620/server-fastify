...
var DEFAULT_SECTOR_SHADOW = {
  color: "rgba(0, 0, 0, 0.3)",
  blur: 8,
  offsetX: 0,
  offsetY: 2
};
var DEFAULT_SECTOR_BORDER = {
...
function resolveDefaults(options) {
  return {
    ...
    sectorStyleConfig: {
      shadow: options.sectorStyle?.shadow ? { ...DEFAULT_SECTOR_SHADOW, ...options.sectorStyle.shadow } : void 0,
      border: options.sectorStyle?.border ? { ...DEFAULT_SECTOR_BORDER, ...options.sectorStyle.border } : void 0
    }
  };
}
...
function drawTurntable(ctx, canvas, state, items, highlightIndex, labelConf, btnConf, sectorGap, innerRatio, sectorStyle) {
  ...
  for (let i = 0; i < sectorCount; i++) {
    ...
    ctx.save();
    if (sectorStyle?.shadow) {
      ctx.shadowColor = sectorStyle.shadow.color;
      ctx.shadowBlur = sectorStyle.shadow.blur;
      ctx.shadowOffsetX = sectorStyle.shadow.offsetX ?? 0;
      ctx.shadowOffsetY = sectorStyle.shadow.offsetY ?? 0;
    }
    ctx.fillStyle = items[i].color;
    ctx.fill();
    ctx.restore();
    if (sectorStyle?.border) {
      ctx.lineWidth = sectorStyle.border.width;
      ctx.strokeStyle = sectorStyle.border.color;
      ctx.stroke();
    }
...
var TurntableSelection = class {
  ...
  constructor(options = {}) {
    ...
    this.sectorStyleConfig = resolved.sectorStyleConfig;
    this.state = {
      centerX: 0,
      centerY: 0,
      radius: 0,
